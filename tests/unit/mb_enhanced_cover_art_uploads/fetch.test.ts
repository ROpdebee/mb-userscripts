...
describe('fetching image contents', () => {
    ...
    it('rejects on HTTP error', async () => {
        ...
        await expect(fetchImageContents(new URL('https://example.com/broken'), 'test.jpg', {}))
    ...
    it('rejects on unsupported provider page', async () => {
        ...
        await expect(fetchImageContents(new URL('https://example.com/not-an-album'), 'test.jpg', {}))
    ...
    it('resolves with fetched image', async () => {
        ...
        await expect(fetchImageContents(new URL('https://example.com/working'), 'test.jpg', {}))
...
describe('fetching images from providers', () => {
    ...
    it('returns no images if provider provides no images', async () => {
        ...
        await expect(fetchImagesFromProvider(new URL('https://example.com'), fakeProvider, false))
            .resolves.toMatchObject({
                images: [],
                containerUrl: {
    ...
    it('allows provider to postprocess images', async () => {
        ...
        await expect(fetchImagesFromProvider(new URL('https://example.com'), provider, false))
            .resolves.toMatchObject({
                images: [{
                    originalUrl: {
    ...
    describe('fetching only front images', () => {
        it('removes non-front images', async () => {
            ...
            await expect(fetchImagesFromProvider(new URL('https://example.com'), fakeProvider, true))
                .resolves.toMatchObject({
                    images: [{
                        originalUrl: {
