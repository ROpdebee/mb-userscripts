...
import type { CoverArtMetadata, FetchedImage, ImageContents, QueuedImage } from '@src/mb_enhanced_cover_art_uploads/types';
...
import { CoverArtDownloader as OriginalCoverArtDownloader } from '@src/mb_enhanced_cover_art_uploads/images/download';
...
// eslint-disable-next-line jest/prefer-jest-mocked -- jest.mocked types don't work because of generic return type.
const mockRequestGet = jest.mocked(request.get) as unknown as jest.Mock<Promise<Response>, [URL | string, unknown]>;
...
class FakeProvider extends CoverArtProvider {
    ...
    public readonly findImages = jest.fn();
...
class CoverArtDownloader extends OriginalCoverArtDownloader {
    public override downloadImage(image: CoverArtMetadata): Promise<FetchedImage | null> {
        return super.downloadImage(image);
    }

    public override downloadImageContents(url: URL, fileName: string, id: number, headers: Record<string, string>): Promise<ImageContents> {
        return super.downloadImageContents(url, fileName, id, headers);
    }
}

type DownloadImageContentsSpy = jest.SpiedFunction<CoverArtDownloader['downloadImageContents']>;
...
function disableDummyFetch(mock: DownloadImageContentsSpy): void {
    // Restore original implementation of downloadImageContents
    mock.mockRestore();
}
...
describe('downloading image contents', () => {
    let downloader: CoverArtDownloader;
    ...
    beforeEach(() => {
        downloader = new CoverArtDownloader(hooks);
        onRetry.mockClear();
    });
    ...
    it('assigns unique ID to each file name', async () => {
        ...
        result = downloader.downloadImageContents(new URL('https://example.com/working'), 'test.jpg', 1, {});
...
describe('downloading image', () => {
    ...
    beforeEach(() => {
        downloader = new CoverArtDownloader(hooks);
        mockFetchImageContents = jest.spyOn(downloader, 'downloadImageContents');
    ...
    describe('without maximisation', () => {
        it('does not maximise the image', async () => {
            const result = downloader.downloadImage({ ...baseCoverArt, originalUrl: new URL('https://example.com/test') });
        ...
        it('uses the URL filename if present', async () => {
            const result = downloader.downloadImage({ ...baseCoverArt, originalUrl: new URL('https://example.com/test.jpg') });
    ...
    describe('with maximisation', () => {
        ...
        it('maximises the image', async () => {
            const result = downloader.downloadImage(coverArt);
        ...
        it('fetches nothing if maximised URL already fetched', async () => {
            await downloader.downloadImage({ ...coverArt, maximisedUrlCandidates: [], originalUrl: new URL('https://example.com/1') });
...
describe('downloading and enqueuing images', () => {
    ...
    beforeEach(() => {
        downloader = new CoverArtDownloader(hooks);
        mockDownloadImageContents = jest.spyOn(downloader, 'downloadImageContents');
    ...
    it('processes single image', async () => {
        ...
        const result = await downloader.fetchAndEnqueueCoverArt(batch);
    ...
    it('does not fetch URL which was already fetched', async () => {
        ...
        await downloader.fetchAndEnqueueCoverArt(batch);

        const result = downloader.fetchAndEnqueueCoverArt(batch);
    ...
    it('does not fetch provider URL which was already fully fetched', async () => {
        ...
        await expect(downloader.fetchAndEnqueueCoverArt(batch))
    ...
    it('does not fetch maximised URL which was already fetched previously', async () => {
        ...
        await downloader.fetchAndEnqueueCoverArt(batch1);

        await expect(downloader.fetchAndEnqueueCoverArt(batch2))
            .resolves.toHaveProperty('images', []);
    });
