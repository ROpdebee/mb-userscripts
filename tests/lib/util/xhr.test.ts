...
describe('gmxhr', () => {
    ...
    it('uses GET by default', async () => {
        mocked_GM_xhr.mockImplementation((options) => options.onload?.(stubResponse));
        await gmxhr('test');
        expect(mocked_GM_xhr).toBeCalledTimes(1);
        expect(mocked_GM_xhr).toBeCalledWith(expect.objectContaining({ method: 'GET' }));
    });
    ...
    it('allows overriding HTTP method', async () => {
        ...
        expect(mocked_GM_xhr).toBeCalledTimes(1);
        expect(mocked_GM_xhr).toBeCalledWith(expect.objectContaining({ method: 'POST' }));
    });
    ...
    it('accepts string URLs', async () => {
        ...
        expect(mocked_GM_xhr).toBeCalledTimes(1);
        expect(mocked_GM_xhr).toBeCalledWith(expect.objectContaining({ url: 'https://example.com/test?x=1#y' }));
    });
