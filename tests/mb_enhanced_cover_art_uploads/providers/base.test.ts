...
describe('cover art providers', () => {
    ...
    describe('merging track images', () => {
        ...
        describe('post-processing to remove duplicates', () => {
            it('retains single-copy images', async () => {
                const fetchResults = [[{
                    // ...
                }, {
                    digest: '123',
                ...
                const result = await provider.postprocessImages(fetchResults);

                expect(result).toBeArrayOfSize(1);
            });

            it('prefers front image if it exists', async () => {
                const fetchResults = [[{
                ...
                expect(result).toBeArrayOfSize(1);
                expect(result[0].types).toStrictEqual([ArtworkTypeIDs.Front]);
            });

            it('merges track numbers', async () => {
                const fetchResults = [[{
                    types: [ArtworkTypeIDs.Track],
                    comment: 'Track 1',
                }, {
                ...
                expect(result).toBeArrayOfSize(1);
                expect(result[0].comment).toBe('Tracks 1, 2, 3');
            });

            it('sorts track numbers before merge', async () => {
                const fetchResults = [[{
                    types: [ArtworkTypeIDs.Track],
                    comment: 'Tracks 2, 3',
                }, {
            ...
            it('supports empty track numbers', async () => {
                const fetchResults = [[{
                    types: [ArtworkTypeIDs.Track],
                }, {
                    digest: '123',
                ...
                expect(result).toBeArrayOfSize(1);
                expect(result[0].comment).toBe('Track 1');
            });

            it('retains remainder of track comment', async () => {
                const fetchResults = [[{
                    types: [ArtworkTypeIDs.Track],
                    comment: 'Tracks 2, 3 - Bandcamp full-size image',
                }, {
                    digest: '123',
                    types: [ArtworkTypeIDs.Track],
                    comment: 'Tracks 2, 3 - Bandcamp full-size image',
                ...
                expect(result).toBeArrayOfSize(1);
                expect(result[0].comment).toBe('Tracks 1, 2, 3 - Bandcamp full-size image');
            });
        });
    });
});
