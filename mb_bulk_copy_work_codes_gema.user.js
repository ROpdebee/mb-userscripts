// ==UserScript==
// @name         MB: Bulk copy-paste work codes w/ GEMA extensions
// @version      2021.5.27#private-alpha
...
const mainUIHTML = `<div id="ropdebee-work-menu"
    ...
    <div class="buttons">
        ...
        <button type="button" id="ROpdebee_MB_Lookup_GEMA"
                title="Lookup full GEMA ID from truncated."
                style="cursor: help;"
            >Lookup GEMA ID</button>
...
class BaseWorkForm {
    ...
    activateButtons() {
        ...
        this.form.querySelector('button#ROpdebee_MB_Lookup_GEMA')
            .addEventListener('click', (evt) => {
                evt.preventDefault();
                this.resetLog();
                this.lookupGEMA();
    ...
    lookupGEMA() {
        let GEMA_IDs = this.existingCodeInputs.filter(({ select, input }) => getSelectedID(select) === 'GEMA ID');
        if (GEMA_IDs.length !== 1) {
            this.log('error', 'Need exactly one GEMA ID');
            return;
        }
        GEMA_IDs = GEMA_IDs.map(({ select, input }) => input.value);
        console.log(GEMA_IDs[0]);

        const GEMA_DATA = `AJAXREQUEST=auswahlForm%3Aj_id29&auswahlForm=auswahlForm&auswahlForm%3AsearchCriterionOption=searchCriterion_workCode&auswahlForm%3AsearchCriterionKindOption=searchCriterionKind_exact&auswahlForm%3AsearchCriterion=${GEMA_IDs[0]}&auswahlForm%3AandSearchCriterionOption=auswahl_auswahl&auswahlForm%3AandSearchCriterionKindOption=searchCriterionKind_exact&auswahlForm%3AandSearchCriterion=&auswahlForm%3ApageSize=pageSizes_PAGE_05&javax.faces.ViewState=j_id1&auswahlForm%3AsearchButton=auswahlForm%3AsearchButton&AJAX%3AEVENTS_COUNT=1&`;

        let that = this;
        GM_xmlhttpRequest({
            method: 'POST',
            url: 'https://online.gema.de/werke/search.faces',
            data: GEMA_DATA,
            headers: {
                Origin: 'https://online.gema.de',
                Referer: 'https://online.gema.de/werke/search.faces',
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            },
            onerror: (err) => that.log('error', error),
            onload: (resp) => {
                let dom = new DOMParser().parseFromString(resp.responseText, 'text/html');
                let codeRows = dom.querySelectorAll('.itemSubTitle');
                if (codeRows.length !== 1) {
                    that.log('error', 'Expected exactly one code');
                    return;
                }

                let code = codeRows[0].querySelector('.workSocworkcde').innerText.split(' ')[1];
                let iswc = codeRows[0].querySelector('.workIswc').innerText.split(' ')[1];

                that.checkAndFill(JSON.stringify({
                    'source': 'GEMA lookup',
                    'iswcs': iswc ? [iswc] : [],
                    agencyCodes: {GEMA: [code]},
                    'title': '',
                }));
            }
        });
