...
(function () {
    ...
    function getExternalLinksEditor(mbInstance) {
        ...
        var editor = (_ref = (_mbInstance$releaseEd = (_mbInstance$releaseEd2 = mbInstance.releaseEditor) === null || _mbInstance$releaseEd2 === void 0 ? void 0 : _mbInstance$releaseEd2.externalLinks.externalLinksEditorRef) !== null && _mbInstance$releaseEd !== void 0 ? _mbInstance$releaseEd : mbInstance.sourceExternalLinksEditor) === null || _ref === void 0 ? void 0 : _ref.current;
        assertHasValue(editor, 'Cannot find external links editor object');
        return editor;
    }
    function getLastInput(editor) {
        var linkInputs = qsa('input.value', editor.tableRef.current);
        return linkInputs[linkInputs.length - 1];
    }
    var initSplitter = _async(function (windowInstance) {
        var editorContainer = qsMaybe('#external-links-editor-container', windowInstance.document);
        if (!editorContainer)
            return;
        return _await(retryTimes(() => getExternalLinksEditor(windowInstance.MB), 100, 50), function (editor) {
            var splitter = new LinkSplitter(editor);
            var _createPersistentChec = createPersistentCheckbox('ROpdebee_multi_links_no_split', 'Don\'t split links', () => {
                    splitter.toggle();
                }), _createPersistentChec2 = _slicedToArray(_createPersistentChec, 2), checkboxElmt = _createPersistentChec2[0], labelElmt = _createPersistentChec2[1];
            splitter.setEnabled(!checkboxElmt.checked);
            insertCheckboxElements(editor, checkboxElmt, labelElmt);
        });
    });
    var submitUrls = _async(function (editor, urls) {
        if (urls.length === 0)
            return;
        var lastInput = getLastInput(editor);
    ...
    class LinkSplitter {
        constructor(editor) {
            ...
            this.patchedOnChange = function () {
                var rawUrl = arguments.length <= 2 ? undefined : arguments[2];
                LOGGER.debug('onchange received URLs '.concat(rawUrl));
                var splitUrls = rawUrl.trim().split(/\s+/);
                if (splitUrls.length <= 1) {
                    _this.originalOnChange(...arguments);
                    return;
                }
                var lastUrl = splitUrls[splitUrls.length - 1];
                var firstUrls = splitUrls.slice(0, -1);
                logFailure(submitUrls(editor, firstUrls).then(() => {
                    var lastInput = getLastInput(_this.editor);
    ...
    function insertCheckboxElements(editor, checkboxElmt, labelElmt) {
        ...
        var lastInput = getLastInput(editor);
        var marginLeft = lastInput.offsetLeft + ((_lastInput$parentElem = (_lastInput$parentElem2 = lastInput.parentElement) === null || _lastInput$parentElem2 === void 0 ? void 0 : _lastInput$parentElem2.offsetLeft) !== null && _lastInput$parentElem !== void 0 ? _lastInput$parentElem : 0);
    ...
    function listenForIframes() {
        var iframeObserver = new MutationObserver(mutations => {
            var _iterator = _createForOfIteratorHelper(mutations.flatMap(mut => [...mut.addedNodes])), _step;
            try {
                for (_iterator.s(); !(_step = _iterator.n()).done;) {
                    var addedNode = _step.value;
                    if (addedNode instanceof HTMLElement && addedNode.classList.contains('iframe-dialog')) {
                        var iframe = qsMaybe('iframe', addedNode);
                        if (iframe) {
    ...
    function attachLinkSplitter() {
        logFailure(initSplitter(window));
        listenForIframes();
    }
    var insertAtisketCopyButtons = (() => {
        function createButton(textGetter) {
            var btn = function () {
                ...
                setStyles$1($$a, { marginLeft: '1ex' });
                var $$b = document.createTextNode('Copy');
                $$a.appendChild($$b);
                return $$a;
            }.call(this);
            return btn;
        ...
        function getArtistLinks() {
            var urls = qsa('.prop.artists a:not(.mb)').map(a => a.href);
            return urls.join('\n');
        }
        function getReleaseLinks() {
            var urls = qsa('section#mba-links a').map(a => a.href);
            return urls.join('\n');
        }
        return () => {
            var _qsMaybe, _qsMaybe2;
            (_qsMaybe = qsMaybe('.prop.artists div')) === null || _qsMaybe === void 0 ? void 0 : _qsMaybe.prepend(createButton(getArtistLinks));
            (_qsMaybe2 = qsMaybe('section#mba-links > h2')) === null || _qsMaybe2 === void 0 ? void 0 : _qsMaybe2.append(createButton(getReleaseLinks));
        };
    })();
    if (document.location.hostname === 'musicbrainz.org' || document.location.hostname.endsWith('.musicbrainz.org')) {
        attachLinkSplitter();
