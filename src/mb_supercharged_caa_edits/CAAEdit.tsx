...
import type { ROpdebee_getDimensionsWhenInView, ROpdebee_getImageDimensions } from '@src/mb_caa_dimensions/exports';
...
import { openComparisonDialog } from './comparisonDialog';
...
export class CAAEdit {
    ...
    private insertRow(header: string, rowContent: HTMLElement | string | Array<HTMLElement | string>): void {
        ...
        this.edit
            ...
            ?.after(<tr><th>{header}</th><td>{...rowContent}</td></tr>);
    }
    ...
    private insertReleaseInfo(): void {
        ...
        const detailsContent = insertBetween(
            [['Status', status], ['Packaging', packaging], ['Format', format]]
                .flatMap(([title, value]) => [`${title}: `, <span data-name={title}>{value}</span>]),
            '; ');
        const eventsContent = insertBetween(
            events.flatMap(([evtDate, evtCountries]) => {
                ...
                return [<span data-name='release-date'>{evtDate}</span>, ` (${countries})`];
            }), '; ');
        const labelsContent = insertBetween(
            [...labels].map(([lblGid, lblName]) => <a href={`/label/${lblGid}`}>{lblName}</a>),
            ', ');
        const identifiersContent = [`Cat#: ${[...catnos].join(', ') || '??'}; Barcode: `, <span data-name='barcode'>{barcode}</span>];
    ...
    private async setImage(): Promise<void> {
        ...
        const img = <img
            ...
            onError={(): void => {
                ...
                const errorMsg = <span className='error' style={{ display: 'block' }}>{errorText}</span>;
