...
(function () {
  ...
  function transformCAAURL(url) {
    ...
    if (urlObj.host === 'coverartarchive.org' && urlObj.pathname.startsWith('/release/')) {
      ...
      return "https://archive.org/download/mbid-".concat(releaseId, "/mbid-").concat(releaseId, "-").concat(imageName);
    }
  ...
  function parseCAAIDs(url) {
    ...
    if (urlObj.host === 'coverartarchive.org' && urlObj.pathname.startsWith('/release/')) {
      ...
      const _urlObj$pathname$spli3 = urlObj.pathname.split('/').slice(2),
            _urlObj$pathname$spli4 = _slicedToArray(_urlObj$pathname$spli3, 2),
            releaseId = _urlObj$pathname$spli4[0],
            thumbName = _urlObj$pathname$spli4[1];
  ...
  class CAAImage extends BaseImage {
    ...
    getDimensions() {
      if (this.imgUrl.endsWith('.pdf')) {
        LOGGER.warn("Cannot get dimensions of PDF, skipping ".concat(this.imgUrl));
  ...
  function createDimensionsString(imageInfo) {
      return typeof imageInfo.dimensions !== 'undefined' ? ''.concat(imageInfo.dimensions.height, 'x').concat(imageInfo.dimensions.width) : 'failed :(';
  }
